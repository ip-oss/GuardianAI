...
This script:
1. Runs 10 new test games with MIXED agent types
2. Each game has 4 agents with different behavioral prompts:
   - 1 Cooperative
   - 1 Greedy
   - 1 Deceptive
   - 1 Random (rotating: free_rider, vengeful, random)
3. Saves trajectories with agent type labels for fingerprinting

Architecture:
- Parallel game execution (same as Phase 1)
- Each game has 4 agents with different behaviors
- 50 rounds per game (matching Phase 1)
"""
...
def run_adversarial_game(game_idx, rounds, provider, results_dir, base_seed=2000):
    """
    Run a single adversarial game with mixed agent types.

    Agent composition per game:
    - Agent 0: Cooperative (baseline)
    - Agent 1: Greedy
    - Agent 2: Deceptive
    - Agent 3: Rotating (free_rider, vengeful, random)
    ...
    try:
        ...
        agent_4_type = rotating_types[game_idx % len(rotating_types)]

        agent_types = {
            'agent_0': 'cooperative',
            'agent_1': 'greedy',
            'agent_2': 'deceptive',
            'agent_3': agent_4_type,
        }

        print(f"[ADVERSARIAL Game {game_idx}] Starting with agents: {list(agent_types.values())}")
        ...
        print(f"[ADVERSARIAL Game {game_idx}] Complete! ({len(trajectories)} trajectories)")
        ...
        print(f"[ADVERSARIAL Game {game_idx}] Error: {e}")
        import traceback
        traceback.print_exc()
...
def main():
    ...
    parser.add_argument('--num-games', type=int, default=10,
                       help='Number of adversarial test games (default: 10)')
    parser.add_argument('--rounds', type=int, default=50,
                       help='Rounds per game (default: 50)')
    parser.add_argument('--provider', type=str, default='deepseek',
                       help='LLM provider')
    parser.add_argument('--max-workers', type=int, default=None,
                       help='Max parallel games (default: all games)')
    args = parser.parse_args()

    num_games = args.num_games
    max_workers = args.max_workers or num_games

    print("=" * 70)
    print("PHASE 2C: ADVERSARIAL TEST GAMES")
    print("=" * 70)
    ...
    print("  - Agent 3: Rotating (free_rider, vengeful, random)")
    print("=" * 70)
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        for future in as_completed(futures):
            ...
            if result['success']:
                ...
                print(f"\n[PROGRESS] {completed}/{num_games} games complete ({elapsed/60:.1f} min)")

    elapsed = time.time() - start_time
    print(f"\nAll games complete in {elapsed/60:.1f} minutes")
    print(f"  Collected: {len(all_trajectories)} trajectories")

    # Save all adversarial trajectories
    all_path = results_dir / "adversarial_trajectories.pkl"
    with open(all_path, 'wb') as f:
        pickle.dump(all_trajectories, f)
    print(f"Saved all trajectories: {all_path}")

    # Save agent type mapping
    agent_types_path = results_dir / "agent_types.json"
    with open(agent_types_path, 'w') as f:
        json.dump(all_agent_types, f, indent=2)
    print(f"Saved agent types: {agent_types_path}")
    ...
    print("\n" + "=" * 70)
    print("SUCCESS!")
    print("Next step: Run Phase 2D to extract fingerprints and classify agents")
    print("=" * 70)
